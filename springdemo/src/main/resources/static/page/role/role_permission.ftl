...
//                    console.log(d);
                    ...
                    var roleList = d.roleSet;
                    console.log("roleid = " + roleId);
                    console.log(roleList);
                    for(var r in roleList){
                        if(roleList[r].id == roleId){
                            isAdd = true;
                            break;
                        }
                    }
        ...
        form.on('switch(locked)', function(obj){
            ...
            var dom = obj.elem
                ...
                ,addUrl = "/admin/role/permissions/add"
                ,delUrl = "/admin/role/permissions/delete";
            console.dir(dom.dataset);
            console.log("isadd = "+isAdd);
            var url = !!isAdd ? delUrl : addUrl;
            $.post(url,{roleId : $('#roleId').val(),permissionId : id},function (data) {
                var msg = "操作成功";
                if(!data.success){
                    msg = data.message;
                }
...
//                layer.alert(JSON.stringify(data));
                var url="/admin/role/permissions/add/list";
                var ids = [];
                for(var d in data){
                    ids.push(data[d].id);
                }

                $.post(url,{roleId : $('#roleId').val(),permissionIdList : ids},function (data) {
                    var msg = "操作成功";
                    if(!data.success){
                        msg = data.message;
                    }
            ...
            ,delCheckData: function(){ //删除选中权限
                var checkStatus = table.checkStatus('list')
                    ,data = checkStatus.data;
                var url="/admin/role/permissions/delete/list";
                var ids = [];
                for(var d in data){
                    ids.push(data[d].id);
                }
